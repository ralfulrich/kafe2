...
class TestXYMultiFitBasicInterface(AbstractTestFit, unittest.TestCase):
    ...
    def test_initial_state(self):
        self.run_test_for_all_fits(
            dict(
                ...
                poi_names=['b', 'c', 'd'],
                # TODO: fix type
                parameter_values=np.array(self._ref_initial_pars),
                # TODO: fix type
    ...
    def test_fit_results(self):
        self.run_test_for_all_fits(
            dict(
                ...
                poi_names=['b', 'c', 'd'],
                # TODO: fix type
                parameter_values=np.array(self._nominal_fit_result_pars),
                # TODO: fix type
        ...
        self.run_test_for_all_fits(
            dict(
                ...
                did_fit=True,
                poi_names=['b', 'c', 'd', 'a'],
                parameter_values=np.array(self._nominal_fit_result_pars_big),
                # TODO: fix type
                poi_values=list(self._nominal_fit_result_pars_big),
                x_data=np.hstack([self._ref_x, self._ref_x]),
                y_data=np.hstack([self._ref_y_data, self._ref_y_data_2]),
                data=np.array([np.hstack([self._ref_x, self._ref_x]),
                               np.hstack([self._ref_y_data, self._ref_y_data_2])]),
        ...
        self.run_test_for_all_fits(
            dict(
                ...
                did_fit=True,
                poi_names=['a', 'b', 'c', 'd'],
                parameter_values=np.array(self._nominal_fit_result_pars_big_reversed),
                # TODO: fix type
                poi_values=list(self._nominal_fit_result_pars_big_reversed),
                x_data=np.hstack([self._ref_x, self._ref_x]),
                y_data=np.hstack([self._ref_y_data_2, self._ref_y_data]),
                data=np.array([np.hstack([self._ref_x, self._ref_x]),
                               np.hstack([self._ref_y_data_2, self._ref_y_data])]),
    ...
    def test_update_data(self):
        ...
        _fit.do_fit()
        _new_estimates = self._nominal_fit_result_pars * 2
        ...
        self._assert_fit_properties(
            ...
            dict(
                ...
                x_data=self._ref_x,
                y_data=self._ref_y_data * 2,
                poi_values=list(_new_estimates),
            ),
            rtol=1e-2
        )
